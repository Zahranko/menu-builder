...
export default function Template1({ site, products }: TemplateData) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fafafa]">
      ...
      {/* ── Header bar ── */}
      <header className="bg-white/90 border-b border-black/[0.06] sticky top-0 z-10 backdrop-blur-md">
        <div className="max-w-2xl mx-auto px-5 h-14 flex items-center justify-between">
          ...
          <a
            href="#products"
            className="text-xs font-medium text-gray-500 hover:text-gray-900 transition-colors hidden sm:block"
          >
            View menu ↓
          </a>
        </div>
      ...
      <section className="relative overflow-hidden" style={heroStyle}>
        {hasBgImage && <div className="absolute inset-0 bg-black/35" />}
        <div className="relative flex flex-col items-center text-center px-6 pt-16 pb-14 gap-6">
          <div className="space-y-3">
            <h1 className={`text-3xl sm:text-5xl font-bold leading-tight tracking-tight ${hasBgImage ? 'text-white drop-shadow-lg' : 'text-gray-900'}`}>
              {site.hero_title || site.business_name || 'Welcome'}
            </h1>
            {site.hero_subtitle && (
              <p className={`text-base sm:text-lg max-w-sm mx-auto leading-relaxed ${hasBgImage ? 'text-white/80' : 'text-gray-500'}`}>
                {site.hero_subtitle}
              </p>
            )}
          </div>
          <a
            href="#products"
            className={`inline-flex items-center gap-2 px-6 py-2.5 rounded-full text-sm font-semibold transition-all hover:scale-105 active:scale-95 ${
              hasBgImage
      ...
      {/* ── Products ── */}
      <main id="products" className="flex-1 max-w-2xl mx-auto w-full px-4 py-10 scroll-mt-14">
        {products.length === 0 ? (
      ...
      <footer className="border-t border-gray-100 bg-white mt-auto">
        <div className="max-w-2xl mx-auto px-5 py-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2.5">
                {site.logo_url ? (
                  <img src={site.logo_url} alt={site.business_name} className="w-8 h-8 rounded-lg object-cover" />
                ) : (
                  <div className="w-8 h-8 rounded-lg bg-gray-900 flex items-center justify-center">
                    <span className="text-white font-bold text-xs">{(site.business_name || 'M').charAt(0).toUpperCase()}</span>
                  </div>
                )}
                <span className="font-semibold text-gray-800 text-sm">{site.business_name || 'Our Menu'}</span>
              </div>
              {site.footer_text && (
                <p className="text-xs text-gray-400 max-w-[200px] leading-relaxed">{site.footer_text}</p>
              )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-6 text-xs text-gray-500">
              {site.footer_hours && (
                <div className="space-y-1">
                  <p className="font-semibold text-gray-700 uppercase tracking-wider text-[10px]">Hours</p>
              ...
              {site.footer_address && (
                <div className="space-y-1">
                  <p className="font-semibold text-gray-700 uppercase tracking-wider text-[10px]">Location</p>
                  <div className="flex items-start gap-1.5">
                    <MapPinIcon />
            ...
            {socialLinks.length > 0 && (
              <div className="space-y-1">
                <p className="font-semibold text-gray-700 uppercase tracking-wider text-[10px]">Follow us</p>
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 flex items-center justify-between">
            <p className="text-xs text-gray-300">Powered by Menu Builder</p>
          </div>
        </div>
      </footer>
    </div>
